 ...
 *     2. modified by Hae Jin Jung on June 4, 2018
...
/**
 *   Given head of the linked list to check that the list is empty.
 *
 *   Input
 *   - NODE *set : the head of the linked list
 */
int isEmpty(NODE *set) {
    if (!set) {
        return 1;
...
/**
 *   Given head of the linked list and data to check that
 *   the data is in the list.
 *
 *   Input
 *   - NODE *set : the head of the linked list
 *   - int i_data : the integer data
 */
int isMember(NODE *set, int i_data) {
    NODE *temp = set;
    if (!temp) {
        fprintf(stderr, "[ERROR] There is not a set.\n");
        return 0;
    }
    while (temp) {
        if (temp->data == i_data) {
...
/**
 *   Given head of the linked list and data to put in the list,
 *   insert data into the linked list.
 *   Insert data in the ordered list of ascending order.
 *
 *   Input
 *   - NODE **p_set : the pointer of the head of the linked list
 *   - int i_data : the integer data
 */
int addElement(NODE **p_set, int i_data) {
    if (!(*p_set)) {
        ...
        *p_set = itemHEAD;
        ...
        while (1) {
            if (i_data < temp->data) {
                ...
                *p_set = itemHEAD;
                return 1;
            } else if (i_data == temp->data) {
    // the function doesn't allocate data in only this condition
                return 0;
            } else {
                if ((i_data > temp->data) && !(temp->link)) {
                    NODE *itemHEAD = (NODE *)mymalloc(sizeof(NODE));
                    if (!itemHEAD) {
                        fprintf(stderr, "[ERROR] Out of memory.\n");
                        return 0;
...
/**
 *   Given head of the linked list and data pointer to delete,
 *   delete data which in the linked list.
 *
 *   Input
 *   - NODE **p_set : the pointer of the head of the linked list
 *   - int *p_i_data : the pointer of the integer data
 */
int deleteElement(NODE **p_set, int *p_i_data) {
    NODE *itemHEAD = *p_set;
    NODE *del_itemHEAD;
    if (!itemHEAD) {
        return 0;
    ...
    // find the data in the list and delete it
        while (itemHEAD) {
            del_itemHEAD = itemHEAD->link;
            if (!del_itemHEAD) {
                if (itemHEAD->data != *p_i_data) {
                    itemHEAD = itemHEAD->link;
                } else {
                    myfree(itemHEAD);
                    *p_set = 0;
                    return 1;
                ...
                if (itemHEAD->data == *p_i_data) {
                    ...
                    *p_set = itemHEAD->link;
                    myfree(itemHEAD);
                    return 1;
                } else if ((itemHEAD->link)->data == *p_i_data) {
                    itemHEAD->link = (itemHEAD->link)->link;
                    myfree(del_itemHEAD);
                    return 1;
                } else {
                    itemHEAD = itemHEAD->link;
...
/**
 *   Given two head of the linked list to make the intersection
 *   of two sets.
 *   Insert data of the intersection in the ordered list of ascending order.
 *
 *   Input
 *   - NODE *set1 : the pointer of the head of the first linked list set
 *   - NODE *set2 : the pointer of the head of the second linked list set
 */
NODE *setIntersection(NODE *set1, NODE *set2) {
    NODE *temp = NULL;
    NODE *tempTAIL = NULL;
    ...
    return saveIntersection;
}

/**
 *   Given two head of the linked list to make the union
 *   of two sets.
 *   Insert data of the union in the ordered list of ascending order.
 *
 *   Input
 *   - NODE *set1 : the pointer of the head of the first linked list set
 *   - NODE *set2 : the pointer of the head of the second linked list set
...
/**
 *   Given head of the linked list to delete the list,
 *   delete whole the linked list.
 *
 *   Input
 *   - NODE **p_set : the pointer of the head of the linked list
 */
void deleteSet(NODE **p_set) {
    NODE *itemHEAD = *p_set;
    if (itemHEAD) {
        while (*p_set) {
            *p_set = itemHEAD->link;
